...
class SSHV2Backend:
    ...
    def __init__(self, upper_layer: Any, config):
        ...
        self.command = config.get("command", default=None)
        ...
        self.password_file = config.get("password_file", default=None)
        if not self.password and self.password_file:
            self.password = self._read_password_file(self.password_file)
        self.use_agent = config.get("use_agent", default=False)
        self.look_for_keys = config.get("look_for_keys", default=True)
        self.auth_debug = config.get("auth_debug", default=False)
    ...
    def get_cached_ssh_password(self):
        """Return the in-memory password that authenticated the SSH session, if any."""
    ...
    def send_data(self, data: bytes) -> None:
        ...
        if self.channel is not None and self.channel.closed:
            self.channel = None

        if self._running and (self._rx_thread is None or not self._rx_thread.is_alive()):
            self.start()
    ...
    def ssh_thread(self) -> None:
        ...
        try:
            ...
            self.channel.get_pty(term="vt100")
