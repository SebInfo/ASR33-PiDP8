...
def register_font(ttf_path: str) -> bool:
    ...
    if sys.platform.startswith("win"):
        ...
        # macOS: font registration is not supported here.
        # The caller will keep using the detected font family name.
        return False
...
class ASR33TkFrontend:
    ...
    def __init__(self, terminal, backend, config, sound=None):
        ...
        self._sound_mute_state = self.cfg.get(
            "sound", "config", "mute_state", default="unmuted"
        )
...
#                print("Registered font family:", family_name)
                ...
                # macOS: register_font() may fail although the font is installed
                # in ~/Library/Fonts. Keep the detected family name and let Tk
    ...
    def _keypress(self, event):
        # macOS/Tkinter peut donner event.char vide pour Return.
        # On force donc l'envoi du retour chariot ASCII.
        if getattr(event, "keysym", "") in ("Return", "KP_Enter"):
            self._backend.send_data(b"\r")
            self.screen_top_lln = None
        ...
        if getattr(event, "keysym", "") in ("BackSpace", "Delete"):
            self._backend.send_data(b"\x7f")
            return "break"
        
        # Gestion explicite des touches Control sous macOS/Tkinter.
        # Ctrl-A -> 0x01, Ctrl-C -> 0x03, Ctrl-U -> 0x15, etc.
        keysym = getattr(event, "keysym", "")
        state = getattr(event, "state", 0)

        # Sur Tk/macOS, le bit Control est généralement 0x4.
        if state & 0x4:
            if len(keysym) == 1 and keysym.isalpha():
                code = ord(keysym.upper()) - ord("@")
                self._backend.send_data(bytes([code]))
                return "break"

            if keysym in ("bracketleft", "Escape"):
                self._backend.send_data(b"\x1b")
                return "break"
