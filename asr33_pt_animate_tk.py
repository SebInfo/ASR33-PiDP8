...
PAPER_COLOR = "#ffeedd"
BUTTON_BG = "#f3dcc6"
BUTTON_ACTIVE_BG = "#e8c9aa"
BUTTON_DISABLED_FG = "#9a8c80"
BUTTON_PRIMARY_BG = "#d8ead2"
BUTTON_STOP_BG = "#efd3cc"
BUTTON_DEVICE_BG = "#d7e4f2"
UNPUNCHED_GHOST_OUTLINE = True
ASCII_CHAR_MASK_MSB = True
BIT_LABEL_BASE = 0  # 0: zero-based (0-7), 1: one-based (1-8)
MAX_LABEL_WIDTH = 26  # Maximum width for status labels
...
class PapertapeViewer(tk.Toplevel):
    ...
    def _make_button(self, parent, text, command, state="normal", bg=BUTTON_BG):
        """Create a consistent, easy-to-click control button."""
        return tk.Button(
            parent,
            command=command,
            bg=bg,
    ...
    def bring_to_front(self) -> None:
        """Raise the tape window and give it focus."""
    ...
    def _setup_ui(self) -> None:
        ...
        self.bt_load = self._make_button(
            control_frame, "Load", self._on_load, bg=BUTTON_DEVICE_BG
        )

        self.bt_unload = self._make_button(
            control_frame, "Unload", self.unload_click, state="disabled"
        )

        self.bt_on = self._make_button(
            control_frame,
        ...
        if self.mode == "reader":
            self.bt_rewind = self._make_button(
                control_frame, "Rewind", self._on_rewind, state="disabled"
            )
            self.bt_rewind.grid(row=1, column=2, padx=2, pady=2, sticky="w")
        else:
            self.bt_mode = self._make_button(
                control_frame, "Mode", self._on_mode
            )
            self.bt_mode.grid(row=1, column=2, padx=2, pady=2, sticky="w")

            self.bt_ptp = self._make_button(
                control_frame, "PTP", self._on_ptp_attach, bg=BUTTON_DEVICE_BG
            )
            self.bt_ptp.grid(row=0, column=2, padx=2, pady=2, sticky="w")
    ...
    def _on_ptp_attach(self) -> None:
        """Handles the SIMH PTP attach button click event."""
        if self.mode != "punch" or not hasattr(self.outer, "attach_ptp"):
            return
        if self.outer.attach_ptp():
            self.set_button_state("ptp")
